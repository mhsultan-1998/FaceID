...
'''
    ...
    In this problem, we will implement the logistic regression method for binary classification problems using
    NumPy.
    
    We will get familiar with gradient computation using the chain rule. 
    
    We will use binary cross entropy as the loss function and stochastic gradient descent to train the model 
    parameters.
...
#----------------------------------------------------
'''
    (Value Forward Function 1) Given a logistic regression model with parameters w and b, compute the linear 
    logit value on a data sample x. 
    ---- Inputs: --------
...
#----------------------------------------------------
'''
    (Local Gradient 1.1) Suppose we are given a logistic regression model with parameters w and b. Suppose we 
    have already computed the linear logit z(x) on a training sample x. Please compute the gradient of the 
    linear logit z(x) with w.r.t. the bias b. 
    ---- Outputs: --------
...
#----------------------------------------------------
'''
    (Local Gradient 1.2) Suppose we are given a logistic regression model with parameters w and b. Suppose we 
    have already computed the linear logit z(x) on a training sample x. Please compute the partial gradients 
    of the linear logit z(x)  w.r.t. the weights w. Here 'w.r.t.' represents 'with respect to'. 
    ---- Inputs: --------
        * x: the feature vector of a data sample, a float numpy vector of length p.
    ---- Outputs: --------
        * dz_dw: the partial gradients of the logit z w.r.t. the weights w, a numpy float matrix of shape 
        (p by 1). It represents (d_z / d_w). The i-th element represents ( d_z / d_w[i]).
    ---- Hints: --------
        * This problem can be solved using 1 line(s) of code.
'''
#---------------------
...
#----------------------------------------------------
'''
    (Value Forward Function 2) Suppose we are given a logistic regression model and we have already computed 
    the linear logit z(x) on a training sample x. Suppose the label of the training sample is y. Please 
    compute the loss function of the logistic regression model on the training sample. 
    ---- Inputs: --------
        * z: the logit value of the instance, a float scalar.
    ...
    ---- Hints: --------
        ...
        * When computing exp(z), you need to be careful about an overflowing case. When the z is a large
          number (say 1000),  the computer can no longer store the result in a floating-point number. In this
          case, we may want to avoid computing exp(z) and assign the final result of L(x) directly. When z is
          very large (say 1000), 1+exp(z) will be very close to exp(z), so we can simplify the equation 
          "log(1+exp(z))-yz" into   "log (exp(z)) - yz". Here log() and exp() cancel out, so we only have 
          "z-yz" left, which is "z(1-y)". 
        * This problem can be solved using 1 line(s) of code.
'''
#---------------------
def compute_L(z, y):
...
#----------------------------------------------------
'''
    (Local Gradient 2) Suppose we are given a logistic regression model and we have already computed the 
    logits z(x) on a training sample x. Suppose the label of the training sample is y. Please compute the 
    gradient of the loss function (L) w.r.t. the linear logit (z) . 
    ---- Inputs: --------
        * z: the logit value of the instance, a float scalar.
        * y: the label of a data sample, an integer scalar value. The values can be 0 or 1.
    ---- Outputs: --------
...
#----------------------------------------------------
'''
    (Global Gradient 1) Suppose we are given a logistic regression model with parameters (w and b) and we have
    a training data sample (x,y).  Suppose we have already computed the local gradients on the data sample. 
    Please compute the global gradient of the loss L w.r.t. bias parameter b using chain rule. 
    ---- Inputs: --------
        * dL_dz: .
        * dz_db: the partial gradient of logit z w.r.t. the bias b, a float scalar. It represents (d_z / d_b).
    ---- Outputs: --------
...
'''
    ...
    a training data sample (x,y).  Suppose we have already computed the local gradients on the data sample. 
    Please compute the global gradient of the loss function L w.r.t. the weight parameters w using chain rule. 
    ---- Inputs: --------
        * dL_dz: .
        * dz_dw: the partial gradients of the logit z w.r.t. the weights w, a numpy float matrix of shape 
          (p by 1). It represents (d_z / d_w). The i-th element represents ( d_z / d_w[i]).
    ---- Outputs: --------
        * dL_dw: the partial gradient of the loss function L w.r.t. the weight vector w, a numpy float vector 
          of length p.  The i-th element represents ( d_L / d_w[i]).
    ---- Hints: --------
        * This problem can be solved using 1 line(s) of code.
'''
#---------------------
...
'''
    ...
    a training data sample (x) with label (y).  Suppose we have already computed the activation a(x) on the 
    data sample in the forward-pass. Please compute the global gradients of the loss w.r.t. the parameters w 
    and b on the data sample using back propagation. 
    ---- Inputs: --------
        * x: the feature vector of a data sample, a float numpy vector of length p.
        * y: the label of a data sample, an integer scalar value. The values can be 0 or 1.
        * z: the logit value of the instance, a float scalar.
...
def backward(x, y, z):
    #########################################
    ## INSERT YOUR CODE HERE (5 points)
    dL_dw, dL_db = np.multiply(compute_dL_dz(z, y), x), np.multiply(compute_dL_dz(z, y), 1)
    #########################################
    return dL_dw, dL_db
    #-----------------
    '''  
...
#----------------------------------------------------
'''
    (Gradient Descent 1) Suppose we are given a logistic regression model with parameters (w and b) and we 
    have a training data sample (x,y).  Suppose we have already computed the gradient of the loss w.r.t. the 
    bias b on the data sample. Please update the bias parameter b using gradient descent. 
    ---- Inputs: --------
        * b: the bias value of the logistic model, a float scalar.
        * dL_db: the partial gradient of the loss function L w.r.t. the bias b, a float scalar.
...
#----------------------------------------------------
'''
    (Gradient Descent 2) Suppose we are given a logistic regression model with parameters (w and b) and we
    have a training data sample (x,y).  Suppose we have already computed the partial gradients of the loss 
    w.r.t. the parameters w on the data sample. Please update the weight parameter w using gradient descent. 
    ---- Inputs: --------
        * w: the weights parameter of the logistic model, a float numpy vector of length p.
        * dL_dw: the partial gradient of the loss function L w.r.t. the weight vector w, a numpy float vector
          of length p.  The i-th element represents ( d_L / d_w[i]).
...
#----------------------------------------------------
'''
    (Training Softmax Regression) Given a training dataset X (features), Y (labels), train the logistic 
    regression model using stochastic gradient descent: iteratively update the weights w and bias b using the 
    gradients on each random data sample.  We repeat n_epoch passes over all the training instances. 
    ---- Inputs: --------
...
#----------------------------------------------------
'''
    (Using Logistic Regression) Given a trained logistic regression model with parameters w and b. Suppose we
    have a test sample x. Please use the logistic regression model to predict the label of x and the 
    probability of the label of x being positive, i.e. the activation a(x). 
    ---- Inputs: --------
        * x: the feature vector of a data sample, a float numpy vector of length p.
        * w: the weights parameter of the logistic model, a float numpy vector of length p.
        * b: the bias value of the logistic model, a float scalar.
...
* L:  the cross entropy loss value, a float scalar. 
* dL_da:  the partial gradient of the loss function L w.r.t. the activation a, a float scalar value. It 
          represents (d_L / d_a). 
* da_dz:  the partial gradient of the activation a w.r.t. the logit z, a float scalar value. It represents 
          (d_a / d_z). 
* dz_dw:  the partial gradients of the logit z w.r.t. the weights w, a numpy float matrix of shape (p by 1). 
          It represents (d_z / d_w). The i-th element represents ( d_z / d_w[i]). 
* dz_db:  the partial gradient of logit z w.r.t. the bias b, a float scalar. It represents (d_z / d_b). 
* dL_dw:  the partial gradient of the loss function L w.r.t. the weight vector w, a numpy float vector of 
          length p.  The i-th element represents ( d_L / d_w[i]). 
* dL_db:  the partial gradient of the loss function L w.r.t. the bias b, a float scalar. 
* alpha:  the step-size parameter of gradient descent, a float scalar. 
...
* X:  the feature matrix of training instances, a float numpy matrix of shape (n by p). 
* Y:  the labels of training instance, a numpy integer vector of length n. The values can be 0 or 1. 
* Ytest:  the predicted labels of test data samples, an integer numpy array of length ntest. Ytest[i] 
          represents the predicted label on the i-th test sample. 
* P:  the predicted probabilities of test data samples to have positive labels, a float numpy vector of length
      n_test.  P[i] is a value is between 0 and 1, indicating the probability of the i-th data sample to have 
      positive label. 
